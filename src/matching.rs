...
pub fn find_topk_with_tolerance(
    ...
    Zip::from(left)
        ...
        .par_for_each(|v_l, mut row_out| {
            ...
            let dist = dist.mapv(|elem| {
                if elem > tolerance {
                    OrderedFloat::from(BAD_VALUE)
                } else {
                    OrderedFloat::from(elem)
                }
            });
...
mod matching_tests {
    ...
    fn find_topk_with_tolerance_test() {
        let v1 = Array1::<f64>::from_vec(vec![100.125, 6.5489, 6.5488, 0.00, -19.0, 77777.777]);
        println!(" {}", v1);
        let v2 = Array1::<f64>::from_vec(vec![
            0.5, 100.01, 99.01, 6.5488, 7.0, 77778.777, 10.0, -20.0, 3.24,
        ]);
        println!(" {}", v2);
